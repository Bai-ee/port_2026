...
plus the throughline across the targets. Founder-ready voice, under 120 words, no markdown.`,
  'opportunity-signals': String.raw`# Opportunity Signals (public buying-signal scan)
...
const RECIPES = {
  ...
  'opportunity-signals': {
    ...
    description: 'Public buying-signal scan — scores social posts (editable query rows) as client opportunities: trigger, likely problem, relevant service, and a non-pitch response angle. Read-only v1.',
    file: 'opportunity-signals.md',
    prompt: EMBEDDED_PROMPTS['opportunity-signals'],
    source: 'internal — Opportunity Signals (Market Signals elevated search)',
    model: DEFAULT_MODEL,
    maxTokens: DEFAULT_MAX_TOKENS,
    // Reads the stored opportunity search pool (marketingBrief.opportunitySignals),
    // not raw agentData. See the recipe-run route's content router.
