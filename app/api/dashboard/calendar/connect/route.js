...
/**
 * POST /api/dashboard/calendar/connect
 * Authenticated. Returns the Google consent URL for the signed-in user's client.
 * The client then redirects the browser to it (keeps auth in the header, not the URL).
 */
export async function POST(request) {
  if (!cal.isConfigured()) return json({ error: 'Google Calendar OAuth is not configured on the server.' }, 503);
  ...
  return json({ url: cal.buildAuthUrl(clientId) });
}
