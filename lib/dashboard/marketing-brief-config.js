...
export function buildDefaultMarketingBriefConfig(client, dashboardState) {
  ...
  return {
    ...
    // Must match the save route's default (marketing-brief/config/route.js:
    // `Number(body?.freshnessDays || 7)`). They disagreed — client 1 vs server
    // 7 — so any save from the Market Signals card silently rewrote a stored 7
    // down to 1, and a 24h window quietly excluded every article older than
    // yesterday. Keep these two numbers in sync.
    freshnessDays: 7,
