...
// Cards whose detail modal is a full-width workspace: the left visual/about
// cell is dropped and the bento grid collapses to one column, so the card gets
// the whole 1280px modal. Everything else keeps the 40% visual + content split.
export const FULL_WIDTH_MODAL_CARD_IDS = new Set([
  'media-library',
  // X Monitor renders a KPI strip, a growth chart, two audience columns, and a
  // wide post table — it needs the full width the same way media-library does.
  'x-monitor',
  // X Calendar lists scanned quote-react candidates, each with post text, a
  // metrics row and a caption textarea — cramped in the 60% split.
  'x-calendar',
  // Archive Inbox lists every confirmed archive asset with its series, rights
  // state and an inline story editor — the same wide-row shape as X Calendar.
  'archive-inbox',
  // Content Engine pairs a day of posting slots with an inventory editor whose
  // form has a full-width story textarea. Both need the width.
  'x-content',
]);
...
export const CARD_ACTION_EDIT = new Set([
  ...
  'knowledge-base', 'client-brain', 'email-digest', 'creative-brief-composer', 'invoice-builder', 'industry', 'create-client',
