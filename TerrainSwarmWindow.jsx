...
import { TerrainSwarm } from './TerrainSwarm';

const DEFAULTS = { size: 100, height: 15, speed: 0.5, ruggedness: 8, bgColor: '#000000', bgAlpha: 0 };

const SLIDERS = [
  { key: 'size',       label: 'Terrain Scale',    min: 10,  max: 200, step: 1   },
  { key: 'height',     label: 'Mountain Height',  min: 1,   max: 50,  step: 0.5 },
  { key: 'speed',      label: 'Undulation Speed', min: 0.1, max: 3.0, step: 0.05 },
  { key: 'ruggedness', label: 'Ruggedness',       min: 1,   max: 20,  step: 0.5 },
...
const TerrainSwarmWindow = ({ style }) => {
  ...
  useEffect(() => {
    ...
    const swarm = new TerrainSwarm(container, 20000);
    swarm.updateParams(DEFAULTS);
  ...
  const panel = panelOpen && createPortal(
    <div style={panelStyle}>
      <div style={panelHeaderStyle}>
        <span style={panelTitleStyle}>Terrain Settings</span>
  ...
  return (
    <>
      ...
      >
        <button type="button" onClick={() => setPanelOpen(o => !o)} style={gearBtnStyle} title="Terrain settings">
...
const panelHeaderStyle      = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.65rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.07)' };
const panelTitleStyle       = { fontSize: '0.68rem', fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'rgba(245,241,223,0.55)' };
const smallBtnStyle         = { background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '0.3rem', color: 'rgba(245,241,223,0.65)', fontSize: '0.65rem', padding: '0.18rem 0.5rem', cursor: 'pointer' };
...
export default TerrainSwarmWindow;
