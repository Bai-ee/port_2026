...
export function createGLTFLoaderBundle({ THREE, stdlib, renderer }) {
  ...
  // CORRECTION (discovered while inspecting a meshopt-compressed local GLB):
  // three-stdlib's MeshoptDecoder is a FACTORY function (`() => API`), not
  // the decoder API itself — passing it uncalled makes
  // GLTFLoader.setMeshoptDecoder's own internal shape check reject it
  // ("setMeshoptDecoder must be called before loading compressed files"),
  // failing every meshopt-compressed GLB, including through the live
  // admin glb-import pipeline this bundle already serves.
