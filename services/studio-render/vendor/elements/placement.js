...
const ELEMENT_ANCHORS = {
  ...
  // 'foreground'/corner (matching glb-import/liquid-glass-lens's own hero-
  // showcase precedent) was tried first and came back infeasible in EVERY
  // format at the real measured bound (1.05 — see catalog.js; the shirt's
  // worst-case wind swing is genuinely almost a full extra body-width,
  // larger than most other single-object showcase types) — checked directly
  // via defaultTransformForFormat, not assumed. Centered/background clears
  // all three formats with no other change, same intentional-overlap
  // treatment as Kinetic Rings/Homepage Particle Hero/Topographic Floor.
  'hanging-tshirt': { xSign: 0, ySign: 0, depth: 'background' },
  // The 3D merch model (Part B) — a static showcase object at the exact
  // same measured local radius as glb-import (0.46, see catalog.js), so it
  // gets the same corner-anchored/foreground treatment, checked directly
  // via defaultTransformForFormat, not assumed. Opposite corner from
  // glb-import's own anchor so the two don't default to overlapping
  // positions if a scene has both.
  'tshirt-model': { xSign: 1, ySign: -1, depth: 'foreground' },
  // A screen showcase object, but at localRadius 0.65 the foreground shell
  // is the same infeasibility trap metaball-bloom (0.55) already hit in
  // Square/Reel — anchored background-corner instead, opposite corner from
  // cloth-banners so a scene with both doesn't default-overlap.
  'device-mockup': { xSign: -1, ySign: -1, depth: 'background' },
  // Centered by explicit owner direction ("layers land around the paper and
  // get moved from there") — intentional-overlap in catalog.js, same
  // treatment as portal-plane/kinetic-rings.
  'image-layer': { xSign: 0, ySign: 0, depth: 'background' },
};
