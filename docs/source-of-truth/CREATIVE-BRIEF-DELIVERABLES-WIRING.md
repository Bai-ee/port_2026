...
3. **Captured vs missing** — a `## CAPTURE STATUS` block states whether the crawl ran, came back thin (JS-rendered site), or never ran, plus which modules failed or never ran. The system prompt forbids reporting a non-captured input as a site gap. A JS-rendered site (client-rendered SPA) that yields a thin static read now gets a rendered fallback (Phase 1, `docs/plans/BRIEF-RENDERED-SCRAPE-SONNET-HANDOFF.md`): `site-fetcher.js` detects a thin page or a sub-10KB empty-hydration-root SPA shell and re-fetches it via Browserless `/content` (`fetchBrowserlessContent`, hard-capped at 4 rendered fetches per run). A successful, content-improving render replaces that page's primary fields (including `siteMeta` for the homepage, which is what fixes the all-✗ social checklist) and is tagged `renderMode: 'rendered-fallback'`; the static read is preserved on `staticView` for the same page. When Browserless is unavailable or the render doesn't improve on the static read, the page stays `renderMode: 'static'` with a `renderFailed` reason and the brief's "could not read" honesty register is unchanged — this is a genuine degrade, not a silent gap.
