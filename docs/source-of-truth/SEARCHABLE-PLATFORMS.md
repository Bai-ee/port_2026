...
- **Search terms must be normalized at every entry point — prose in `categoryTerms` silently kills a client's platform sections** (2026-08-20 clairecalls incident). The scout config generator accepted the LLM's `categoryTerms` array with no per-term validation, and the Market Signals save route's old `splitTerms` did `String(input).split(/[\n,]+/)` — an ARRAY input got comma-joined then re-split, shredding an LLM prose paragraph into junk fragments (`"walking"`, `"or commuting checking in on a parent…"`) while keeping a 40-word blob as one "term". Reddit/IG queries built from that vocabulary returned zero results, the relevance filter (`filterRelevantSignals`) had no usable category terms, and every platform section in the client's email/report rendered its empty state — looking like a pipeline failure when it was config garbage. Fix: `features/scout-intake/search-term-normalizer.js` (`normalizeSearchTerms` — splits arrays AND strings on `\n,;`, strips leading conjunctions, drops any term over 4 words / 40 chars as prose, dedupes, caps), now wired into BOTH entry points: the generator's `brandKeywords`/`competitors`/`categoryTerms` capability defaults and the `marketing-brief/config` save route's `splitTerms`. A new term-list entry point (import, migration script, another route) must go through `normalizeSearchTerms` too. Regression test: `features/scout-intake/__tests__/search-term-normalizer.test.js` reproduces the exact prod data. Existing clients provisioned before the fix may still carry malformed terms — symptom: platform searches persistently return 0 items for a client whose Test-button raw output also looks empty or off-topic; check `client_configs.{id}.marketingBriefConfig.categoryTerms` for paragraph-length entries (clairecalls was hand-repaired 2026-08-20).
